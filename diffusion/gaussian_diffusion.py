...
class GaussianDiffusion:
    ...
    def training_losses(
        self, model, x_start, t, model_kwargs=None, noise=None, use_cd_xyz_loss=False, use_cd_color_loss=False
    ) -> Dict[str, th.Tensor]:
        """
        ...
        if self.loss_type == "kl" or self.loss_type == "rescaled_kl":
            ...
            model_output, prev_latent = model(x_t, t, **model_kwargs)
            if isinstance(model_output, tuple):
                model_output, extra = model_output
            else:
                extra = {}
        ...
        if isinstance(extra, dict) and "losses" in extra:
        ...
        # === Chamfer Losses for Predicted x_start ===
        if self.model_mean_type == "epsilon" and (use_cd_xyz_loss or use_cd_color_loss):
            rc_scale = _extract_into_tensor(self.sqrt_alphas_cumprod, t, [x_t.shape[0]]).to(x_t)

            x_start_pred = self._predict_xstart_from_eps(x_t, t, model_output)

            if use_cd_xyz_loss:
                terms["c_dist"] = rc_scale * self.chamfer_distance_xyz(x_start_pred, x_start)
                terms["loss"] += terms["c_dist"]

            if use_cd_color_loss:
                terms["c_dist_color"] = rc_scale * self.chamfer_distance_color(x_start_pred, x_start)
                terms["loss"] += terms["c_dist_color"]
    ...
    def chamfer_distance_xyz(self,
        ...
        dist = th.cdist(xyz1, xyz2, p=2).pow(2)
    ...
    def chamfer_distance_color(self,
        p1: th.Tensor,
        p2: th.Tensor
    ):
        """
        Squared-L2 Chamfer distance on RGB only.

        Args:
        p1, p2: [B, C=6, N] tensors.  Uses only channels 3:6, assumes colors in [-1,1].
        Returns:
        [B]-shaped tensor of Chamfer distances.
        """
        assert p1.dim() == 3 and p2.dim() == 3
        B, C1, N1 = p1.shape
        _, C2, N2 = p2.shape
        assert C1 == 6 and C2 == 6, "Color Chamfer needs exactly 6 channels (XYZ+RGB)"
        assert p1.size(0) == p2.size(0)

        # [B, N1, 3] and [B, N2, 3]
        col1 = p1[:, 3:, :].transpose(1, 2)
        col2 = p2[:, 3:, :].transpose(1, 2)

        # compute pairwise color distances and square
        dist = th.cdist(col1, col2, p=2).pow(2)

        min1 = dist.min(dim=2)[0]  # [B, N1]
        min2 = dist.min(dim=1)[0]  # [B, N2]

        cd_color = min1.mean(dim=1) + min2.mean(dim=1)
        return cd_color


    def chamfer_distance_both(self,
        p1: th.Tensor,
        p2: th.Tensor):
        """
        Returns (cd_xyz, cd_color) for two [B,6,N] point clouds.
        """
        cd_xyz   = self.chamfer_distance_xyz(p1, p2)
        cd_color = self.chamfer_distance_color(p1, p2)
        return cd_xyz, cd_color
