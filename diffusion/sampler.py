...
class PointCloudSampler:
    ...
    def sample_batch_progressive(
        self, batch_size: int, model_kwargs: Dict[str, Any], x_target: torch.Tensor = None,
    ...
    def _uncond_guide_model(
        ...
        def model_fn(x_t, ts, model_kwargs):
            ...
            B = half.shape[0]  # Original batch size (before doubling)
            ts = ts[:B]
            ...
            cond_eps, prev_latent_cond = model(half, ts, **kwargs_cond)
            ...
            uncond_eps, prev_latent_uncond = model(half, ts, **kwargs_uncond)
